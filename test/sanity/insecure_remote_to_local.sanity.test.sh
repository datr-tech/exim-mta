...
function test_sanity_insecure_remote_to_local() {
  ...
  local -r sanity_test_name="test_sanity_insecure_remote_to_local"
  local -i -r sanity_test_timeout=10
  ...
  local -i -r mail_server_port="${BOOTSTRAP_EXIM_UNSECURE_PORT}"
  ...
  local -r email_address_from=${BOOTSTRAP_EMAIL_REMOTE}
