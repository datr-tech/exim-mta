...
declare    TEST_NAME="tls_on_connect_auth_positive"
declare -i TIMEOUT=2
...
declare -a FIXTURES=(
  #
  # 'On connect' (submissions) should be successful with PORT 465, TLS v1.2 and AUTH LOGIN
  #
  "${RCPT_ADDR_LOCAL}  ${DELIM} ${RCPT_USER} ${DELIM} ${SNDR_LOCAL} ${DELIM} ${AUTH_TYPE_LOGIN} ${DELIM} ${TLS_VER_1_2} ${DELIM} ${PORT_TLSC} ${DELIM} ${EXIT_CODE_SUCCESS}"
  "${RCPT_ADDR_DOMAIN} ${DELIM} ${RCPT_USER} ${DELIM} ${SNDR_LOCAL} ${DELIM} ${AUTH_TYPE_LOGIN} ${DELIM} ${TLS_VER_1_2} ${DELIM} ${PORT_TLSC} ${DELIM} ${EXIT_CODE_SUCCESS}"

  #
  # 'On connect' (submissions) should be successful with PORT 465, TLS v1.2 and AUTH PLAIN
  #
  "${RCPT_ADDR_LOCAL}  ${DELIM} ${RCPT_USER} ${DELIM} ${SNDR_LOCAL} ${DELIM} ${AUTH_TYPE_PLAIN} ${DELIM} ${TLS_VER_1_2} ${DELIM} ${PORT_TLSC} ${DELIM} ${EXIT_CODE_SUCCESS}"
  "${RCPT_ADDR_DOMAIN} ${DELIM} ${RCPT_USER} ${DELIM} ${SNDR_LOCAL} ${DELIM} ${AUTH_TYPE_PLAIN} ${DELIM} ${TLS_VER_1_2} ${DELIM} ${PORT_TLSC} ${DELIM} ${EXIT_CODE_SUCCESS}"

  #
  # 'On connect' (submissions) should be successful with PORT 465, TLS v1.3 and AUTH LOGIN
  #
  "${RCPT_ADDR_LOCAL}  ${DELIM} ${RCPT_USER} ${DELIM} ${SNDR_LOCAL} ${DELIM} ${AUTH_TYPE_LOGIN} ${DELIM} ${TLS_VER_1_3} ${DELIM} ${PORT_TLSC} ${DELIM} ${EXIT_CODE_SUCCESS}"
  "${RCPT_ADDR_LOCAL}  ${DELIM} ${RCPT_USER} ${DELIM} ${SNDR_LOCAL} ${DELIM} ${AUTH_TYPE_LOGIN} ${DELIM} ${TLS_VER_1_3} ${DELIM} ${PORT_TLSC} ${DELIM} ${EXIT_CODE_SUCCESS}"

  #
  # 'On connect' (submissions) should be successful with PORT 465, TLS v1.3 and AUTH PLAIN
  #
  "${RCPT_ADDR_LOCAL}  ${DELIM} ${RCPT_USER} ${DELIM} ${SNDR_LOCAL} ${DELIM} ${AUTH_TYPE_PLAIN} ${DELIM} ${TLS_VER_1_3} ${DELIM} ${PORT_TLSC} ${DELIM} ${EXIT_CODE_SUCCESS}"
  "${RCPT_ADDR_LOCAL}  ${DELIM} ${RCPT_USER} ${DELIM} ${SNDR_LOCAL} ${DELIM} ${AUTH_TYPE_PLAIN} ${DELIM} ${TLS_VER_1_3} ${DELIM} ${PORT_TLSC} ${DELIM} ${EXIT_CODE_SUCCESS}"
)
...
function test_tls_on_connect_auth_positive() {
  ...
  do
    ...
    exit_code_found=$?
    sleep 0.5
    mbox_contents_found=$(cat "${rcpt_mbox_path}" 2> /dev/null)

    #
    # ASSERT
    #
    assert_same       "${exit_code_expected}" "${exit_code_found}"
    assert_contains   "${msg_subject}"        "${mbox_contents_found}"
    assert_not_empty  "${mbox_contents_found}"
